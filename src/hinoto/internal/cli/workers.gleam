...
import hinoto/internal/cli/prompt
import hinoto/internal/cli/utils
import simplifile
import snag
import tom
...
pub fn load_project_config() {
  ...
  let toml_path = filepath.join(pwd, utils.project_config)
  use toml_text <- result.try(
    simplifile.read(toml_path) |> snag.map_error(simplifile.describe_error),
  )
  tom.parse(toml_text) |> snag.map_error(utils.describe_tom_parse_error)
}
...
pub fn get_project_name() {
  ...
  use project_name <- result.try(
    ...
    |> snag.map_error(utils.describe_tom_get_error),
  )
